	...
	if(request.getMethod().equals("POST")){		
		...
		if(json==null){
			...
			// DeleteDiaryDAO deleteDAO = new DeleteDiaryDAO();
			 ...
			 //파일 생성만 했을 경우 (이미지 파일 생성이 되고 실사용 사진 저장 폴더에 옮기기 전에 있는 upload 폴더에 있는 사진들을 삭제)
			 if(photoFileCreate){
				 //업로드 파일 삭제
				files = FileManager.MakeImages(multiBackup);
				FileManager.DeleteUploadImages(files);
			 }
			 //기존 파일로 백업 (사진 백업은 사실상 현재 안됨 기존 이미지가 이미 대체되었거나 삭제되었기 때문에 기존 사진을 백업 폴더로 넣어놓고 작업을 해야함 그다음 오류가 있을시 백업폴더에 이미지를 가져오고 오류가 없을시 백업 폴더에 사진을 삭제해야함)
